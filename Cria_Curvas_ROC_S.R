...
Dados <- read.csv2('Teste\\Dados\\Segundo.csv', sep = ',', dec = '.') %>%
...
Grafico <- Dados %>% ggplot() +
    ...
    labs(title = 'Nível de raridade = 0.1')

saveRDS(Grafico, 'Teste\\Gráficos\\Segundo.RDS')
...
write.csv2(Sensibilidade, 'Teste\\Tabelas\\Sensibilidade_S.csv', row.names = FALSE)
...
write.csv2(Especificidade, 'Teste\\Tabelas\\Especificidade_S.csv', row.names = FALSE)
...
roc_curve(Pred_ADASYN, Teste, Pred_Prob, event_level = 'second') %>%
...
Grafico_SMOTE <- Curva_ROC_Padrao %>%
    ...
    labs(x = 'TFP', y = 'TVP', title = 'Nível de raridade = 0.1') +

    theme(legend.position = 'bottom')

Grafico_SMOTE %>%

    saveRDS('Teste\\Curva ROC\\SMOTE_S.RDS')
...
Grafico_BSMOTE %>%
    ...
    saveRDS('Teste\\Curva ROC\\BSMOTE_S.RDS')
...
Grafico_ADASYN %>%
    ...
    saveRDS('Teste\\Curva ROC\\ADASYN_S.RDS')
