...
Dados <- read.csv2('Teste\\Dados\\Terceiro.csv', sep = ',', dec = '.') %>%
...
Grafico <- Dados %>% ggplot() +
    ...
    labs(title = 'Nível de raridade = 0.05')

saveRDS(Grafico, 'Teste\\Gráficos\\Terceiro.RDS')
...
write.csv2(Sensibilidade, 'Teste\\Tabelas\\Sensibilidade_T.csv', row.names = FALSE)
...
write.csv2(Especificidade, 'Teste\\Tabelas\\Especificidade_T.csv', row.names = FALSE)
...
Grafico_SMOTE <- Curva_ROC_Padrao %>%
    ...
    labs(x = 'TFP', y = 'TVP', title = 'Nível de raridade = 0.05') +

    theme(legend.position = 'bottom')

Grafico_SMOTE %>%

    saveRDS('Teste\\Curva ROC\\SMOTE_T.RDS')
...
Grafico_BSMOTE %>%
    ...
    saveRDS('Teste\\Curva ROC\\BSMOTE_T.RDS')
...
Grafico_ADASYN %>%
    ...
    saveRDS('Teste\\Curva ROC\\ADASYN_T.RDS')
